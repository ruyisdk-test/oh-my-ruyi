...
def test_frozen_entrypoint_preserves_system_exit_message(monkeypatch, capsys) -> None:
    from oh_my_ruyi import __main__ as entrypoint

    def child_main(_argv: list[str]) -> None:
        raise SystemExit("usage: child CONFIG_PATH")

    monkeypatch.setattr(
        entrypoint.importlib,
        "import_module",
        lambda _name: SimpleNamespace(main=child_main),
    )

    result = entrypoint._run_embedded_command(
        ["oh-my-ruyi", "-m", "oh_my_ruyi.processes.repo_update_child"]
    )

    assert result == 1
    assert capsys.readouterr().err == "usage: child CONFIG_PATH\n"
...
def test_script_entrypoint_uses_absolute_imports(monkeypatch) -> None:
    called_with: list[list[str]] = []
    monkeypatch.setattr(
        "oh_my_ruyi.gui.app.run",
