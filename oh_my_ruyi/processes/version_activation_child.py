"""Run privileged ruyi version activation operations in a child process.

The GUI invokes this adapter through ``sudo -S`` when the managed activation
link is not user-writable.  The service layer still owns all filesystem
validation and mutation; this module only translates command-line arguments
and returns the resulting state as JSON.
"""
...
from ..services.version_manager import activate_version, deactivate_version
