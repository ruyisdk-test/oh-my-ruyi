...
class RuyiAdapter(IRuyiAdapter):
    def __init__(self, config: GlobalConfig, mr: CompositeRepo):
        self.config = config
        self.mr = mr

    def list_devices(self) -> list[DeviceChoice]:
        return list_devices(self.mr)

    def list_variants(self, device_id: str) -> list[VariantChoice]:
        dev = self.mr.get_device(device_id)
        if dev is None:
            return []
        return list_variants(self.mr, dev)

    def list_combos(self, variant_id: str) -> list[ComboChoice]:
        var = self.mr.get_device_variant(variant_id)
        if var is None:
            return []
        return list_combos(self.mr, var)

    def list_package_version_selections(
        self, combo_id: str
    ) -> list[PackageVersionSelection]:
        combo = self.mr.get_image_combo(combo_id)
        if combo is None:
            return []
        return list_package_version_selections(
            self.config, self.mr, combo_package_atoms(combo)
        )
    ...
    def prepare_provision(
        ...
        return prepare_provision(self.config, self.mr, packages)
